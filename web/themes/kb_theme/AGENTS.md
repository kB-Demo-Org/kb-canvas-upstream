# KbTheme Theme - Agent Rules

This document contains coding rules and conventions for the KbTheme theme that AI agents should follow when making changes.
...
**Note**: Even though Drupal core templates sometimes use `<div{{ attributes }}>`, this pattern is not allowed in KbTheme theme components.
...
```twig
{% include '@kb_theme/components/icon/icon.twig' with {
  weight: 'bold',
  icon: icon
} %}
```
...
**Rule**: After completing any changes to the KbTheme theme, always run `npm run format` and `npm run build` to ensure code is properly formatted and the build artifacts are up to date.
...
**Note**: These commands should be run from the KbTheme theme directory (`web/themes/custom/kb_theme/`). Running these commands ensures that:
